...
const mockStudents = [
  {
    id: 1,
    name: "Alice Johnson",
    completedCourses: ["Introduction to React", "Web Design Fundamentals"],
  },
  {
    id: 2,
    name: "Bob Smith",
...
export default function AdminDashboard() {
  ...
  const handleStudentSelect = async (studentName) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5001/findProjectPair",
        {
          name: studentName,
        }
      );
  ...
  const handlePairing = async () => {
    try {
      const response = await axios.post(
        "http://localhost:5001/PairforProject",
        {
  ...
  const getProjects = useCallback(async () => {
    try {
      const response = await axios.post("http://localhost:5001/getProjects", {
  ...
  const getStudents = useCallback(async () => {
    try {
      const response = await axios.get("http://localhost:5001/getAllStudents");
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      setIsLoading(true);
      try {
        const response = await axios.post("http://localhost:5001/addProject", {
          title: values.title,
          description: values.description,
          expectedDeadline:values.deadline,
          token: localStorage["authToken"],
        });
        console.log(response);
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Enter the details of your new project. Our AI will suggest
                    required skills based on your description.
                  </DialogDescription>
                </DialogHeader>
                ...
                <Link to={`/project/${project._id}`}>
                  <Card key={project._id}>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Status:{" "}
                        {project.assignedStudents.length > 0 ? `Taken` : `Open`}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
